...
export async function GET(
  ...
  try {
    ...
    response.headers.set('Content-Disposition', `attachment; filename="shipping-label-${shipmentData.tracking_id}.pdf"`);
...
function renderShippingLabel(doc: jsPDF, shipmentData: any, qrCodeDataUrl: string) {
  try {
    ...
    // Tracking number
    doc.setFontSize(12);
    yPos = centerText(`Código de seguimiento: ${shipmentData.tracking_id}`, yPos, 12);
    yPos += 5;
    ...
    const paymentText = (() => {
      switch(shipmentData.payment) {
        ...
        case 'NO_PAGO': return 'NO PAGADO';
        case 'PAGA_DESTINO': return 'PAGO EN DESTINO';
        default: return shipmentData.payment;
      }
    })();
    ...
    const senderName = shipmentData.sender_name || 
                       (shipmentData.transportist ? shipmentData.transportist.name : 'Remitente');
    ...
    // Sender address
    doc.text(`${shipmentData.origin_street} ${shipmentData.origin_street_number}`, margin, yPos);
    yPos += 5;
    if (shipmentData.origin_floor || shipmentData.origin_apartment) {
      doc.text(`Piso ${shipmentData.origin_floor || '-'}, Dpto ${shipmentData.origin_apartment || '-'}`, margin, yPos);
      yPos += 5;
    }
    doc.text(`${shipmentData.origin_city}, ${shipmentData.origin_state} ${shipmentData.origin_zip_code}`, margin, yPos);
    yPos += 10;
    ...
    const receiverName = shipmentData.receiver_name || 'Destinatario';
    ...
    // Receiver address
    doc.text(`${shipmentData.destination_street} ${shipmentData.destination_street_number}`, margin, yPos);
    yPos += 5;
    if (shipmentData.destination_floor || shipmentData.destination_apartment) {
      doc.text(`Piso ${shipmentData.destination_floor || '-'}, Dpto ${shipmentData.destination_apartment || '-'}`, margin, yPos);
      yPos += 5;
    }
    doc.text(`${shipmentData.destination_city}, ${shipmentData.destination_state} ${shipmentData.destination_zip_code}`, margin, yPos);
    yPos += 10;
    ...
    doc.text(`Fecha de creación: ${creationDate.toLocaleDateString('es-AR')}`, margin, yPos);
    yPos += 5;
    
    // Format the state date
    const stateDate = new Date(shipmentData.state_date);
    doc.text(`Fecha de estado: ${stateDate.toLocaleDateString('es-AR')}`, margin, yPos);
    yPos += 5;
    ...
    if (shipmentData.packages && shipmentData.packages.length > 0) {
      ...
      shipmentData.packages.forEach((pkg: any, index: number) => {
        doc.text(`Paquete ${index + 1}: ${formatPackageType(pkg.package_type)}, ${pkg.weight} kg`, margin, yPos);
        yPos += 5;
        if (pkg.height && pkg.width && pkg.length) {
          doc.text(`Dimensiones: ${pkg.height} × ${pkg.width} × ${pkg.length} cm`, margin + 10, yPos);
          yPos += 5;
        }
      });
    }
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    const trackUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/tracking?tracking_id=${shipmentData.tracking_id}`;
